...
PLANS = {
    ...
    "student_monthly": {"name": "应届生月度会员", "price": 990, "requires_verify": True},
}
...
# Student verification store
student_verifications: dict = {}
...
async def send_verification(req: VerifyEmailRequest):
    """发送学生认证验证码到 .edu.cn 邮箱"""
    email = req.email.strip().lower()
    if not email.endswith(".edu.cn"):
        raise HTTPException(400, "请使用学校邮箱（@xxx.edu.cn）")
    
    code = ''.join(random.choices(string.digits, k=6))
    student_verifications[email] = {"code": code, "verified": False}
    logger.info(f"Student verify code for {email}: {code}")
    
    return {"message": "验证码已发送", "email": email, "debug_code": code}

@router.post("/verify-code")
async def verify_code(req: VerifyCodeRequest):
    """验证学生身份"""
    email = req.email.strip().lower()
    record = student_verifications.get(email)
    if not record:
    ...
    return {"message": "验证成功", "email": email}

@router.get("/verify-status/{email}")
async def verify_status(email: str):
    """查询学生认证状态"""
    record = student_verifications.get(email.strip().lower(), {})
    return {"verified": record.get("verified", False)}

class CheckoutRequest(BaseModel):
    plan_id: str
    plan_id: str  # pro_monthly / proplus_monthly / ...
    email: str = ""
...
async def create_payment(req: CheckoutRequest):
    ...
    if plan.get("requires_verify"):
        if not req.email or not student_verifications.get(req.email.strip().lower(), {}).get("verified"):
            raise HTTPException(400, "请先完成学生身份认证")
    if not plan:
